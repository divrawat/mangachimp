    ...
    return (
        <>
            ...
            <article>
                ...
                <div className='max-w-[1000px] mx-auto mt-8 bg-[whitesmoke] rounded shadow px-3 '>
                    <h1 className={`${roboto.className} text-center font-bold text-3xl pt-6 pb-5 uppercase text-base-color`}>{`${manga?.manga?.fullname} ${manga?.manga?.type}`}</h1>
                    <img className="mx-auto" width={300} height={300} src={`${manga?.manga?.photo}`} alt="Manga Cover" />
                    <p className="my-5 leading-[2] md:px-6 px-2 text-center text-base-text-color">{manga?.manga?.description}</p>
                    <div className="flex flex-wrap justify-center gap-5 px-4">
                        {manga?.manga?.categories?.map((category, index) => (
                            <Link prefetch={false} href={`${DOMAIN}/categories/${category?.slug}?page=1`} key={index} className="text-white bg-base-color  font-bold py-2 px-4 rounded hover:scale-110 active:scale-95 transition-transform">
                                {category?.name}
                            </Link>
                        ))}
                    </div>
                    ...
                    <div className="flex justify-center items-center md:gap-[150px] sm:gap-[100px] pb-8 gap-10 flex-wrap mt-14 text-base-text-color">
                ...
                <div className={`${roboto.className} text-3xl font-bold tracking-wider text-center md:mt-[100px] mt-10 mb-5 text-base-color`}>{`Read ${manga?.manga?.name} ${manga?.manga?.type}`}</div>
                ...
                <div className="mt-10 max-w-[1100px] mb-10 mx-auto px-5 flex flex-wrap justify-center">
                    {chapterArray?.map((chapternumber, index) => (
                        <div className="flex hover:scale-105 active:scale-95 transition-transform" key={index}>
                            <Link prefetch={false} href={`${DOMAIN}/manga/${mangaurl}/chapter-${chapternumber}`} className="p-5 hover:underline text-base-color">
                                <p className="w-[200px] bg-[whitesmoke] text-center p-5 rounded shadow font-bold break-words">
                ...
                <div className="max-w-[1200px] mx-auto mt-10">
                    <h2 className={`${roboto.className} text-center text-3xl font-bold pb-10 text-base-color`}>Related</h2>
                            ...
                            <div className="hover:scale-110 transition-transform border rounded shadow text-center w-[180px] bg-white" key={index}>
                                <Link prefetch={false} href={`${DOMAIN}/manga/${manga?.slug}`}>
                                    ...
                                    <div className='px-2 py-3'>
                                        <div className={`${roboto2.className} text-base-color text-[15px] font-semibold mb-1 text-wrap break-words`}>{manga?.name}</div>
                                        <p className="text-[13px] my-2 py-1 text-base-text-color font-bold">{`Total Chapters:  ${manga?.totalChapters ?? 0}`}</p>
                                    </div>
                                </Link>
                            </div>
                        ))}
                ...
                <div className='max-w-[800px] mx-auto mt-10 px-5 '>
                    {paragraphs?.map((paragraph, index) => (
                        <p key={index} className=' py-6 tracking-wider leading-8 text-[15px] text-base-text-color'>{paragraph}</p>
                    ))}
                </div>
